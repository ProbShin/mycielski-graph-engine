...
protected:
    virtual void std_propagate(unordered_map<INT, vector<INT>>&G, vector<INT>&list_of_nodes, INT steps);
    virtual void rnd_propagate(unordered_map<INT, vector<INT>>&G, vector<INT>&list_of_nodes, INT steps);
private:
    reset();
};
